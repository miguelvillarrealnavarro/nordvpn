		...
		function stopLoading(bool){
			$('#loading').attr('src', 'assets/clear.gif');
			var str = $('#checkStatus').html();
			$('#checkStatus').html(str.replace('Processing','Stopped'));
	  ...
	  text: "Thanks For Using John Wick Checker !",
	  html: true,
		...
		function GbrnTmfn(lstMP, curMP, delim, cEmail, bank, card, info, no){
			...
			ajaxCall = $.ajax({
				...
				beforeSend: function (e) {
					updateTitle('['+no+'/'+lstMP.length+'] Processing ');
					$('#checkStatus').html(''+ lstMP[curMP]).effect("highlight", {color:'#00ff00'}, 1000);
					$('#loading').attr('src', 'assets/loading.gif');
				},
	  ...
	  allowOutsideClick: true,
	  type: "error",
	  text: "Please Input Your email|password !",
	  confirmButtonText: "Continue"
});
					return false;
				}
