		...
		function GbrnTmfn(lstMP, curMP, delim, cEmail, bank, card, info, no){
			...
			ajaxCall = $.ajax({
				...
				beforeSend: function (e) {
					updateTitle('['+no+'/'+lstMP.length+'] Loading....');
		...
		$(document).ready(function(){
			...
			$('#submit').click(function(){
				...
				$('#mailpass').val(mailpass.join("\n")).attr('disabled', true).html('Generating Socks');
				//$('#mailpass').val('Loading.....').attr('disabled', true);
				$('#result').fadeIn(1000);
				resetResult();
				$('#submit').attr('disabled', true);
				$('#stop').val(mailpass.join("\n")).attr('disabled', false);
